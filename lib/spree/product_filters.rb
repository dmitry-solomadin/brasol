module Spree
  ...
  module ProductFilters
    ...
    def ProductFilters.taxons_below(taxon)
      Spree::Product.add_search_scope :taxons_id_in_tree_any do |*opts|
        taxons = Spree::Taxon.where("id IN (#{opts.join(",")})")
        taxons.each do |t|
          opts = (opts << t.descendants.collect {|d| d.id}).flatten
        end
        Spree::Product.where("`spree_taxons`.`id` IN (#{opts.join(",")})")
      end

      return Spree::ProductFilters.all_taxons if taxon.nil?
      { :name   => I18n::t(:sub_categories),
